...
export const FeatureCard = ({ icon, title, description, variant = "light" }: FeatureCardProps) => {
	...
	return (
		<motion.div
			...
			className={`group relative flex flex-col items-center text-center space-y-4 rounded-2xl p-8 transition-shadow duration-300 ${
				isDark
		...
		>
			<div
				className={`rounded-xl p-4 transition-colors duration-300 ${
					isDark
						? "bg-white/10 group-hover:bg-[#d9230f]/20"
						: "bg-[#d9230f]/10 group-hover:bg-[#d9230f]/15"
				}`}
			>
				{icon}
			</div>
...
export const NumberStepCard = ({ step, title, description }: NumberStepCardProps) => {
	return (
		<motion.div
			whileHover={{ y: -4 }}
			transition={{ type: "spring", stiffness: 300, damping: 20 }}
			className="relative flex flex-col items-start rounded-2xl bg-white border border-gray-200 p-8 hover:shadow-xl hover:shadow-black/5 transition-shadow duration-300"
		>
			<div className="text-5xl font-black text-[#d9230f]/20 mb-3">{step}</div>
