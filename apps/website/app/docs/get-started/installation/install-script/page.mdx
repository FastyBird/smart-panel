...
<Callout type="info">
	...
	Available flags: `--port <port>`, `--version <ver>`, `--beta`, `--alpha`, `--with-tailscale`
</Callout>

Add `--with-tailscale` to also install [Tailscale](https://tailscale.com) for remote access, left disabled
until you enable it from the admin UI:

```bash copy
curl -fsSL https://get.smart-panel.fastybird.com | sudo bash -s -- --with-tailscale
```

A Tailscale install failure is reported in the completion banner but never aborts the rest of the install.
See [Remote Access](/docs/admin-management/remote-access) for the setup walkthrough.

<Callout type="warning">
	Installed before privileged remote-access setup (Tailscale's **Set up** step) existed? The sudoers grant
	it needs is added by the installer's own setup step, so an installation that predates it is missing that
	grant. Re-run it once to pick it up — safe to repeat, and it won't touch your existing data or
	configuration:
	...
	See [Set up cannot run](/docs/admin-management/remote-access#set-up-cannot-run) if **Set up** still
	refuses afterward.
</Callout>
...
The script auto-detects the platform (Raspberry Pi and ARM64 boards via flutter-pi, Linux x64 desktop, etc.) and installs
...
<Callout type="info">
	...
	- `--platform <type>` — force a specific platform: `flutterpi`, `linux`
</Callout>
