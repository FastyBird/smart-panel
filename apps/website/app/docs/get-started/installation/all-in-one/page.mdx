...
<Steps>
	...
	<Callout type="info">
		InfluxDB is fully optional. Without it, Smart Panel uses in-memory storage — device values and real-time
		updates work normally, but historical data (charts, timeseries) is not persisted across restarts.
		You can install InfluxDB later without reinstalling Smart Panel.
	</Callout>
...
# Check status
sudo systemctl status smart-panel smart-panel-display
```
...
1. Check Node.js version: `node --version` (must be 24+)
2. Verify the database exists: `ls /var/smart-panel/`
3. Check backend logs: `sudo journalctl -u smart-panel -n 50`
