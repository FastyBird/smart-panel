...
can have higher latency than a local connection. OAuth credentials remain backend-only and are excluded from ordinary
database queries, but the current database columns are not encrypted. Treat the database and its backups as sensitive;
neither the browser nor the panel display can read the credentials.
...
key nor cloud OAuth tokens, but system backups include the resolved credentials needed to restore the integration.
Smart Panel backups are not encrypted by the application.
