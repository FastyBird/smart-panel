...
The experimental Homey integration connects Smart Panel to a Homey Pro, Homey Self-Hosted Server (SHS), or a selected
Homey through Homey Cloud. It imports logical devices already managed by Homey, lets an administrator review and adopt
them, keeps their state synchronized, and sends supported controls back to Homey.
...
Local and Homey Cloud connections use the same discovery, adoption, synchronization, and control paths. Smart Panel
does not pair, rename, move, or remove devices in Homey. Cloud mode additionally requires an installation-owned Homey Web
API client and Homey approval; Smart Panel never asks a user to paste Homey account credentials.

## Before You Start

For a local connection, you need:
...
For a cloud connection, you instead need:

- an exact browser-reachable HTTPS Smart Panel origin;
- an installation-owned confidential Homey Web API client with the exact Smart Panel callback registered;
- Homey Cloud access and an adequate user limit approved for that client; and
- the client ID, client secret, and callback URL ready to enter in the Homey plugin's Admin UI configuration.
...
## Configure a Homey Cloud Connection

Homey Cloud avoids a direct network route between Smart Panel and Homey, but it requires working internet access and
can have higher latency than a local connection. OAuth access and refresh tokens remain backend-only, are excluded from
ordinary database queries, and are encrypted at rest with authenticated encryption. The encryption key is derived from
the write-only client secret saved in the Homey plugin configuration. Replacing that secret invalidates an existing
grant and requires authorization again. Neither the browser nor the panel display can read the secret or token values.

1. In Homey Developer Tools, register a confidential Web API client. Register this exact callback, replacing the
   placeholder with the final Smart Panel origin:

   ```text
   https://<exact-smart-panel-origin>/api/v1/plugins/devices-homey/oauth/callback
   ```

2. Request the required Homey Cloud access and user-limit increase. New clients are limited to 100 Homey Pro users;
   Homey Cloud access is not automatic.
3. Open **Config → Plugins → Homey** and choose **Cloud**. Smart Panel proposes a redirect URL from the Admin origin in
   your browser; register that exact URL in Homey Developer Tools, then enter the client ID and client secret. The
   Admin server must forward its `/api` route to the Smart Panel backend. The redirect remains editable for installations
   with a different public origin. The client secret field is write-only: leaving it blank later preserves the stored
   value.
4. Configure every proxy, ingress, and observability layer to omit the callback query string from access logs. The
   backend removes it before application logging, but cannot redact logs made upstream. See the
   [Homey Cloud OAuth deployment record](https://github.com/FastyBird/smart-panel/blob/main/docs/homey-cloud-oauth.md)
   for the nginx example and complete registration checklist.
5. Save the Homey plugin configuration before selecting **Connect Homey Cloud**. No backend restart or environment-file
   change is required.
6. Complete Homey's consent page. Smart Panel returns to a clean configuration URL. If the account contains multiple
   eligible Homeys, select the exact Homey to use and confirm the selection.
7. Enable and save the plugin, test the saved configuration, and confirm that **Connection status** becomes
   **Connected**.

**Reconnect** stages a replacement grant and keeps the active connection until the new grant succeeds. **Disconnect**
deletes the local token set and tears down the cloud connector. Because Homey's published API does not document a
general token-revocation endpoint, also revoke the grant in Homey account controls if credentials may be compromised.
...
Treat a Smart Panel system backup as a credential. Configuration APIs and the Admin UI expose neither the local Homey
key nor cloud OAuth tokens. Cloud token fields are encrypted in the database, but backups can still contain the local
Homey key, encrypted grants, and other sensitive configuration. Smart Panel does not encrypt the complete backup.
...
- For cloud mode, disconnect and revoke the Homey grant, then complete a fresh authorization after the replacement
  host is trusted.
- Restore the exact admin-managed Homey Cloud client secret with a cloud grant. Changing it invalidates and clears
  the stored grant, after which an administrator must authorize Homey Cloud again.
...
In local mode, the key is missing, revoked, or invalid. Create or copy a new key, replace the saved value, and run the
saved connection test. In cloud mode, choose **Reconnect** and repeat Homey consent; if that fails, verify the backend
client configuration and Homey approval. The integration avoids rapid reconnect attempts after an authentication
failure.
...
- Live local validation currently covers SHS over HTTP `4859`, not Homey Pro hardware or HTTPS `4860`.
- Cloud mode has complete credential-free connector and authorization tests, but live cloud inventory, event, command,
  reconnect, rate-limit, and latency evidence remains pending a dedicated approved client and test grant.
