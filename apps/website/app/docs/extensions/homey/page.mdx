...
The Homey integration connects Smart Panel to a Homey Self-Hosted Server (SHS) over its local API. It imports logical
devices already managed by Homey, lets an administrator review and adopt them, keeps their state synchronized, and
sends supported controls back to Homey.

<Callout type="info">
  **Local Homey SHS support is validated over HTTP port 4859.** Basic reads and
  a passive SDK session were verified on SHS 13.4.0. The complete validation
  matrix passed on SHS 13.4.1, covering sanitized inventory, capability writes
  and ordered events, credential rotation and permission failures,
  startup/restart/network recovery, physical/Homey/Flow changes, concurrent
  commands, disposable-device lifecycle, thermostat setpoint and mode control,
  and clean plugin/backend shutdown. Homey Pro hardware and HTTPS were not
  validated, and physical availability manager events were not emitted by the
  disposable test driver. Periodic reconciliation remains the fallback for
  missed availability events.
</Callout>

Smart Panel does not pair, rename, move, or remove devices in Homey. The integration uses Homey's local API and never
asks a user to enter Homey account credentials.

## Before You Start

For a local connection, you need:

- a configured [Homey Self-Hosted Server](https://support.homey.app/hc/en-us/articles/23975096785948-How-to-install-Homey-Self-Hosted-Server-on-Linux);
...
2. Enter the local API URL and the new API key.
3. Leave the connection timeout and reconciliation interval at their defaults unless your network needs tuning.
4. Choose **Test entered URL and key**. This test is available only when both fields contain a complete candidate
   connection.
5. Save the form and enable the plugin.
6. Confirm that **Connection status** changes to **Connected** and identifies the expected Homey.
...
Support depends on both Homey's device class and its standardized capability IDs. The supported local integration includes:
...
| Thermostats                 | Current temperature, shared target setpoint, and configured heat, cool, auto, and off modes |
...
<Callout type="info">
  Homey exposes one shared thermostat target, which Smart Panel projects into
  its heating and cooling setpoints. Configured heat, cool, auto, and off modes
  are controllable. Smart Panel does not infer actual heating or cooling
  activity from the configured mode; activity is shown only when Homey provides
  a dedicated authoritative status signal.
</Callout>
...
Treat a Smart Panel system backup as a credential. Configuration APIs and the Admin UI do not expose the local Homey
key, but backups can still contain the key and other sensitive configuration. Smart Panel does not encrypt the complete backup.
...
- Homey Cloud is not supported because Homey's Web API authorization is partner-only and Smart Panel does not depend on a hosted bridge.
...
- Actual thermostat heating/cooling activity requires a dedicated authoritative Homey status capability; configured
  mode alone is never presented as activity.
