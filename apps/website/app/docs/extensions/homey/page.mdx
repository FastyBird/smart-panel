...
does not pair, rename, move, or remove devices in Homey. Cloud mode additionally requires a deployment-owned Homey Web
...
- a deployment-owned confidential Homey Web API client with the exact Smart Panel callback registered;
- Homey Cloud access and an adequate user limit approved for that client; and
- the client ID, client secret, and callback URL provisioned only to the Smart Panel backend.
...
the deployment-owned `FB_HOMEY_CLOUD_CLIENT_SECRET`, which must be kept outside the database and stable while a grant is
active. Neither the browser nor the panel display can read the credentials.
...
3. Provision `FB_HOMEY_CLOUD_CLIENT_ID`, `FB_HOMEY_CLOUD_CLIENT_SECRET`, and `FB_HOMEY_CLOUD_REDIRECT_URL` to the
   backend. The redirect value must exactly match the registered callback. Never put the secret in browser-visible
   configuration, an image, or the repository.
...
5. Restart the backend, open **Config → Plugins → Homey**, choose **Cloud**, and save the form before selecting
   **Connect Homey Cloud**.
...
- For cloud mode, disconnect and revoke the Homey grant, then complete a fresh authorization after the replacement
  host is trusted.
- Restore the exact deployment-owned Homey Cloud client secret with a cloud grant. Changing it invalidates and clears
