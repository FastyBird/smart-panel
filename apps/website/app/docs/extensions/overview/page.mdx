...
| [**Remote access**](/docs/admin-management/remote-access) | Internal/external URL registry, proxy trust, and remote-access provider status |
...
| [**Homey SHS**](/docs/extensions/homey) | Connects to a local Homey Self-Hosted Server for reviewed device adoption, real-time synchronization, availability handling, and supported controls |
...
Connect the installation to a private network or tunnel so it's reachable from outside the LAN. See
[Remote Access](/docs/admin-management/remote-access) for setup and concepts.

| Plugin | Description |
|--------|-------------|
| **Tailscale** | Connects the installation to a Tailscale tailnet — sign in, optional HTTPS via Serve, optional public exposure via Funnel |
