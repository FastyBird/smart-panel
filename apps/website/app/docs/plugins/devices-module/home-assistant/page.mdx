...
You can configure the Home Assistant plugin in two ways:

### 1. Directly in the Admin App

- Navigate to Settings → Plugins → Home Assistant in the admin interface.
- Enter your API Key and Hostname in the provided form.
- Changes are saved automatically and applied after a backend restart.
...
<Steps>
	...
	The token will only be shown once. Copy it immediately and paste it into plugin configuration in the admin app or into your configuration file under `/opt/smart-panel/var/data/config.yaml`. Treat it like a password.

	<p align="center">
		<img src="/images/config-plugins.png" alt="Edit plugin configuration" />
	</p>
