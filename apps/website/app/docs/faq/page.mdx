...
If you changed the credentials during flashing (e.g., using Raspberry Pi Imager), use your custom credentials.
...
```bash copy
sudo systemctl stop smart-panel
sudo systemctl stop smart-panel-display
```

2. Delete the config and database files (adjust the path for your install method):

```bash copy
# For one-liner / npm installs:
sudo rm /var/lib/smart-panel/config/config.yaml
sudo rm /var/lib/smart-panel/data/database.sqlite

# For tarball installs:
# sudo rm /var/smart-panel/config/config.yaml
# sudo rm /var/smart-panel/data/database.sqlite
```
...
- Join our community on [Discord](https://discord.gg/HPRJ2GzK)
