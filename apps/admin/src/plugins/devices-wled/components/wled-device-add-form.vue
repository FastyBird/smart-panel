...
import type { IWledDeviceAddFormProps } from './wled-device-add-form.types';

defineOptions({
	name: 'WledDeviceAddForm',
});

const props = withDefaults(defineProps<IWledDeviceAddFormProps>(), {
...
const rules = reactive<FormRules<IWledDeviceAddForm>>({
	...
	hostname: [{ required: true, message: t('devicesWledPlugin.fields.devices.hostname.validation.required'), trigger: 'change' }],
});
