...
import { useCloudflareTunnelSetup } from './useCloudflareTunnelSetup';
...
describe('useCloudflareTunnelSetup', () => {
	beforeEach(() => {
		install.mockReset();
		get.mockReset();
		data.value = null;
	...
	afterEach(() => {
		// The poll handle is a module-level singleton (mirrors `useTailscaleSetup.spec.ts`) - stop
		// it and restore real timers so a poll left running by one test can never bleed into the
		// next.
		useCloudflareTunnelSetup().stopPolling();
		vi.useRealTimers();
	});

	it('does not poll when no status has been loaded yet', () => {
		const { isPolling } = useCloudflareTunnelSetup();
	...
	it('starts polling once install() resolves and the store status turns running', async () => {
		...
		const { install: doInstall, isPolling } = useCloudflareTunnelSetup();

		expect(isPolling.value).toBe(false);

		await doInstall();
		// `install()` itself never updates `data.setup` (that only ever comes from `GET /status` or
		// a `Setup.Progress` websocket event) - simulate the next status read/event doing so.
		data.value = { setup: { state: 'running' } };
		await nextTick();
	...
	it('stops polling when the caller stops it explicitly (mirrors the wizard unmounting)', () => {
		data.value = { setup: { state: 'running' } };
		const { isPolling, stopPolling } = useCloudflareTunnelSetup();
	...
	it('exposes progress from the store `Setup.Progress` websocket event', () => {
		...
		const { progress } = useCloudflareTunnelSetup();
	...
	it('exposes isInstalling from the store semaphore', () => {
		...
		const { isInstalling } = useCloudflareTunnelSetup();
