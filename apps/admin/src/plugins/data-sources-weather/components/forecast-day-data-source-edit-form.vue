...
import type { IForecastDayDataSourceEditForm } from '../schemas/data-sources.types';

defineOptions({
	name: 'ForecastDayDataSourceEditForm',
});
...
const { model, formEl, formChanged, submit, formResult } = useDataSourceEditForm<IForecastDayDataSourceEditForm>({ dataSource: props.dataSource });

const { locations, fetchLocations, areLoading: loadingLocations } = useLocations();

const rules = reactive<FormRules<IForecastDayDataSourceEditForm>>({
	dayOffset: [
