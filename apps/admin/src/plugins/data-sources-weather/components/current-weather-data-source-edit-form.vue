...
import type { ICurrentWeatherDataSourceEditForm } from '../schemas/data-sources.types';

defineOptions({
	name: 'CurrentWeatherDataSourceEditForm',
});
...
const { model, formEl, formChanged, submit, formResult } = useDataSourceEditForm<ICurrentWeatherDataSourceEditForm>({ dataSource: props.dataSource });

const { locations, fetchLocations, areLoading: loadingLocations } = useLocations();

const rules = reactive<FormRules<ICurrentWeatherDataSourceEditForm>>({
