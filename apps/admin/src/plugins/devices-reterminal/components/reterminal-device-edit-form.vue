<template>
	...
	>
		<el-form-item
			:label="t('devicesReTerminalPlugin.fields.devices.id.title')"
			prop="id"
		>
			<el-input
				v-model="model.id"
				:placeholder="t('devicesReTerminalPlugin.fields.devices.id.placeholder')"
				name="id"
				readonly
				disabled
			/>
...
import type { IReTerminalDeviceEditFormProps } from './reterminal-device-edit-form.types';

defineOptions({
	name: 'ReTerminalDeviceEditForm',
});

const props = withDefaults(defineProps<IReTerminalDeviceEditFormProps>(), {
...
const { categoriesOptions, model, formEl, formChanged, submit, formResult } = useDeviceEditForm({
	device: props.device as IReTerminalDevice,
});
...
const rules = reactive<FormRules<IReTerminalDeviceEditForm>>({
