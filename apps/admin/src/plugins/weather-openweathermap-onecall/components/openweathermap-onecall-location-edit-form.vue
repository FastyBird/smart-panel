...
import type { IOpenWeatherMapOneCallLocationEditFormProps } from './openweathermap-onecall-location-edit-form.types';

defineOptions({
	name: 'OpenWeatherMapOneCallLocationEditForm',
});

const props = withDefaults(defineProps<IOpenWeatherMapOneCallLocationEditFormProps>(), {
...
const onSubmit = async (): Promise<void> => {
	...
	try {
		...
		flashMessage.success(t('weatherOpenweathermapOnecallPlugin.messages.locations.edited', { location: model.name }));
		emit('update:remote-form-result', FormResult.OK);
	} catch {
		flashMessage.error(t('weatherOpenweathermapOnecallPlugin.messages.locations.notEdited', { location: model.name }));
