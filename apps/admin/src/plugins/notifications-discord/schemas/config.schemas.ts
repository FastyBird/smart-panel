...
import { isValidDiscordWebhookUrl } from './discord-webhook-url.schemas';

export const NotificationsDiscordConfigEditFormSchema = ConfigPluginEditFormSchema.extend({
	...
	if (typeof value.webhookUrl === 'string' && value.webhookUrl.trim() !== '' && !isValidDiscordWebhookUrl(value.webhookUrl)) {
		context.addIssue({
			code: 'custom',
			path: ['webhookUrl'],
			message: 'Discord webhook URL must start with https:// and must not embed credentials',
		});
	}
});
