...
import type { ISceneTileAddForm } from '../schemas/tiles.types';
import { TILES_SCENE_TYPE } from '../tiles-scene.constants';

defineOptions({
	name: 'SceneTileAddForm',
});

const props = withDefaults(defineProps<ITileAddFormProps>(), {
...
const { model, formEl, formChanged, submit, formResult } = useTileAddForm<ISceneTileAddForm>({
	id: props.id,
...
const rules = reactive<FormRules<ISceneTileAddForm>>({
