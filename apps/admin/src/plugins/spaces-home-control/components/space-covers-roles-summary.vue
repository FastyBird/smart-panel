<template>
	<template v-if="hasTargets">
		...
		>
			{{ t('spacesHomeControlPlugin.detail.coversRoles.title') }}
		</dt>
		<dd class="col-start-2 b-b b-b-solid m-0 p-2 flex items-center justify-between min-w-[8rem]">
				...
				>
					<template #reference>
						...
						>
							<div class="flex items-center gap-1">
								...
								{{ t(`spacesHomeControlPlugin.coversRoles.${summary.role}`) }}
								<el-badge
					...
					<div class="space-y-2">
						<el-alert
							...
							class="mb-2!"
						>
							{{ t(`spacesHomeControlPlugin.coversRoles.descriptions.${summary.role}`) }}
						</el-alert>
						<div class="text-sm font-medium mb-1">
							{{ t('spacesHomeControlPlugin.detail.coversRoles.coversAssigned') }}:
						</div>
			...
			<div v-else class="flex items-center gap-2 text-gray-400 text-sm">
				...
				{{ t('spacesHomeControlPlugin.detail.coversRoles.noRolesAssigned') }}
			</div>
			<el-button
				text
				size="small"
...
import { CoversRole, SPACES_MODULE_PREFIX } from '../../../modules/spaces/spaces.constants';
