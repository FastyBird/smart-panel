...
defineOptions({
	name: 'OpenMeteoLocationAddForm',
});

const props = withDefaults(defineProps<IOpenMeteoLocationAddFormProps>(), {
...
const onSubmit = async (): Promise<void> => {
	...
	try {
		...
		flashMessage.success(t('weatherOpenMeteoPlugin.messages.locations.created', { location: model.name }));
		emit('update:remote-form-result', FormResult.OK);
	} catch {
		flashMessage.error(t('weatherOpenMeteoPlugin.messages.locations.notCreated', { location: model.name }));
