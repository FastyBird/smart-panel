...
describe('HomeyConfigEditFormSchema', () => {
	...
	])('accepts %s so runtime readiness can decide whether to start', (_label, overrides) => {
		expect(HomeyConfigEditFormSchema.safeParse(createConfig(overrides)).success).toBe(true);
	});

	it('rejects a whitespace key replacement', () => {
		const result = HomeyConfigEditFormSchema.safeParse(createConfig({ apiKey: '   ', apiKeyConfigured: true }));

		expect(result.success).toBe(false);
		if (!result.success) {
			expect(result.error.issues).toEqual(expect.arrayContaining([expect.objectContaining({ path: ['apiKey'] })]));
		}
	});

	it('rejects an invalid local URL and normalizes a cleared URL', () => {
		expect(HomeyConfigEditFormSchema.safeParse(createConfig({ url: 'file:///local-path' })).success).toBe(false);
		const cleared = HomeyConfigEditFormSchema.safeParse(createConfig({ url: '   ' }));
		expect(cleared.success).toBe(true);
		if (cleared.success) expect(cleared.data.url).toBeNull();
	});
