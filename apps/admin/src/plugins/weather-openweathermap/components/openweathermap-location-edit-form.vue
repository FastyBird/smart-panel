...
import type { IOpenWeatherMapLocationEditFormProps } from './openweathermap-location-edit-form.types';

defineOptions({
	name: 'OpenWeatherMapLocationEditForm',
});

const props = withDefaults(defineProps<IOpenWeatherMapLocationEditFormProps>(), {
...
const getInitialModel = (): ILocationModel => {
	...
	return {
		...
		locationType: (location.locationType as (typeof OpenWeatherMapLocationType)[keyof typeof OpenWeatherMapLocationType]) || OpenWeatherMapLocationType.lat_lon,
		latitude: (location.latitude as number) ?? null,
		longitude: (location.longitude as number) ?? null,
		cityName: (location.cityName as string) || '',
		countryCode: (location.countryCode as string) || '',
		cityId: location.cityId !== null && location.cityId !== undefined ? String(location.cityId) : '',
		zipCode: (location.zipCode as string) || '',
		zipCountryCode: (location.countryCode as string) || '',
	};
};
...
const onSubmit = async (): Promise<void> => {
	...
	try {
		...
		flashMessage.success(t('weatherOpenweathermapPlugin.messages.locations.edited', { location: model.name }));
		emit('update:remote-form-result', FormResult.OK);
	} catch {
		flashMessage.error(t('weatherOpenweathermapPlugin.messages.locations.notEdited', { location: model.name }));
