<template>
	<app-bar-heading teleport>
		...
		<template #title>
			{{ t('pagesCardsPlugin.headings.pages.editDataSource') }}
		</template>

		<template #subtitle>
			{{ t('pagesCardsPlugin.subHeadings.pages.editDataSource') }}
		</template>
	</app-bar-heading>
	...
	<div
		...
		:element-loading-text="t('pagesCardsPlugin.texts.dataSources.loadingDataSource')"
...
import {
	...
	type FormResultType,
	type IDataSource,
	type IDataSourcePluginsComponents,
	type IDataSourcePluginsSchemas,
	useDataSource,
	useDataSourcesPlugins,
} from '../../../modules/dashboard';
import { RouteNames } from '../pages-cards.contants';

import type { IViewDataSourceEditProps } from './view-data-source-edit.types';

defineOptions({
	name: 'ViewCardsPageDataSourceEdit',
});

const props = defineProps<IViewDataSourceEditProps>();
...
useMeta({
	title: t('pagesCardsPlugin.meta.pages.editDataSource.title'),
});
...
const formSchema = computed<typeof DataSourceEditFormSchema>((): typeof DataSourceEditFormSchema => {
	return element.value?.schemas?.dataSourceEditFormSchema ?? DataSourceEditFormSchema;
});
...
const breadcrumbs = computed<{ label: string; route: RouteLocationResolvedGeneric }[]>(
	(): { label: string; route: RouteLocationResolvedGeneric }[] => {
		...
		items.push({
			label: t('pagesCardsPlugin.breadcrumbs.pages.editDataSource', { page: props.page?.title }),
			route: router.resolve({ name: RouteNames.PAGE_EDIT_DATA_SOURCE, params: { id: props.page?.id, dataSourceId: dataSource.value?.id } }),
		});
...
watch(
	...
	(val: IDataSource | null): void => {
		if (val !== null) {
			...
			flashMessage.warning(t('pagesCardsPlugin.messages.dataSources.deletedWhileEditing'), { duration: 0 });
			if (props.page?.id) {
				if (isLGDevice.value) {
					router.replace({ name: RouteNames.PAGE, params: { id: props.page.id } });
				} else {
