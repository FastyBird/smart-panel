<template>
	<div
		v-loading="isLoading || configAudio === null"
		:element-loading-text="t('configModule.texts.loadingAudioConfig')"
	>
...
import type { ViewConfigAudioProps } from './view-config-audio.types';

defineOptions({
	name: 'ViewConfigAudio',
});

const props = withDefaults(defineProps<ViewConfigAudioProps>(), {
...
const { configAudio, fetchConfigAudio, isLoading } = useConfigAudio();
...
onBeforeMount(async (): Promise<void> => {
	fetchConfigAudio().catch((error: unknown): void => {
...
useMeta({
	title: t('configModule.meta.configAudio.title'),
});
</script>
