...
import { ElDivider, ElForm, ElFormItem, ElInput, ElOption, ElOptionGroup, ElSelect, type FormRules } from 'element-plus';

import { Icon } from '@iconify/vue';

import { useSceneAddForm } from '../../composables/composables';
import { FormResult, type FormResultType } from '../../scenes.constants';
import type { ISceneAddForm } from '../../schemas/scenes.types';

import type { ISceneAddFormProps } from './scene-add-form.types';

defineOptions({
	name: 'SceneAddForm',
});

const props = withDefaults(defineProps<ISceneAddFormProps>(), {
...
const { categoriesOptions, spacesOptionsGrouped, model, formEl, formChanged, submit, formResult, loadingSpaces, fetchSpaces } = useSceneAddForm({
	id: props.id,
});
...
const rules = reactive<FormRules<ISceneAddForm>>({
