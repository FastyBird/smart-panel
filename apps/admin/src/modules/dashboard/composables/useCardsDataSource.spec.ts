...
describe('useCardsDataSource', () => {
	...
	it('fetches cards', async () => {
		const { fetchCards } = useCardsDataSource('page-1');
		await fetchCards();
	...
	it('returns only non-draft cards', () => {
		const { cards } = useCardsDataSource('page-1');
	...
	it('resets filters correctly', () => {
		const { filters, resetFilter } = useCardsDataSource('page-2');
	...
	it('resets page on filter change', async () => {
		const { filters, paginatePage } = useCardsDataSource('page-2');
