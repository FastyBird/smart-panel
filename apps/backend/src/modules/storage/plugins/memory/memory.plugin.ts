import { IQueryOptions, IResults, ISchemaOptions } from 'influx';
...
export class MemoryStoragePlugin implements StoragePlugin {
	...
	writePoints(points: import('influx').IPoint[]): Promise<void> {
		this.getStore().writePoints(points);

		return Promise.resolve();
	}

	query<T>(query: string, _options?: IQueryOptions): Promise<IResults<T>> {
		try {
			const results = this.getParser().execute<T>(query);

			return Promise.resolve(results as unknown as IResults<T>);
		} catch (error) {
	...
	queryRaw<T>(_query: string, _options?: IQueryOptions): Promise<T> {
		return Promise.resolve({ results: [] } as T);
	}

	registerSchema(_schema: ISchemaOptions): void {
