...
@Injectable()
@ValidatorConstraint({ name: 'TileTypeValidation', async: true })
export class TileTypeConstraintValidator implements ValidatorConstraintInterface {
...
export const ValidateTileType = (validationOptions?: ValidationOptions) => {
	return function (object: object, propertyName: string) {
		registerDecorator({
			name: 'ValidateTileType',
			target: object.constructor,
			propertyName,
			options: validationOptions,
			constraints: [],
			validator: TileTypeConstraintValidator,
		});
	};
};
