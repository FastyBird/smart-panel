...
const INDEPENDENT_ALTERNATIVE_NAMED_OUTCOME_SUBJECT_PATTERN = new RegExp(
	String.raw`^(?:[\p{Letter}\p{Number}'’-]+\s+){1,4}(?:(?:already|currently|still)\s+)?${OUTCOME_PREDICATE_PATTERN_SOURCE}\b`,
	'u',
);
...
function hasIndependentAlternativeSubject(
	...
	return (
		...
		INDEPENDENT_ALTERNATIVE_NAMED_OUTCOME_SUBJECT_PATTERN.test(subject) ||
