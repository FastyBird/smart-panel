...
const CONDITIONAL_OUTCOME_ACTION_COMPLEMENT_PATTERN_SOURCE = String.raw`(?:active|blue|brighter|closed|cooler|dimmer|down|eco|green|higher|inactive|locked|lower|off|on|open|red|unlocked|up|warmer|white|(?:at|by|to)\s+(?:[-+]?\d+(?:\.\d+)?|(?:(?:minus|negative)\s+)?${CONDITIONAL_OUTCOME_SPOKEN_SCALAR_VALUE_PATTERN_SOURCE})\s*(?:%|celsius|degrees?|fahrenheit|percent|°\s*(?:c|f))?)`;
