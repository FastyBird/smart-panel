...
const CONDITIONAL_OUTCOME_SUBJECT_PATTERN_SOURCE = String.raw`(?!(?:because|so|to)\b|${CONDITION_PATTERN.source}|(?:${COMPOUND_CONNECTOR_PATTERN_SOURCE})\b)(?:(?:a|an|her|his|its|my|our|the|their|this|these|those|your)\s+${CONDITIONAL_OUTCOME_SUBJECT_TOKEN_PATTERN_SOURCE}(?:\s+${CONDITIONAL_OUTCOME_SUBJECT_TOKEN_PATTERN_SOURCE}){0,3}|(?!(?:${CONDITIONAL_OUTCOME_BARE_SUBJECT_COMMAND_PATTERN_SOURCE})\b)${CONDITIONAL_OUTCOME_SUBJECT_TOKEN_PATTERN_SOURCE}(?:\s+${CONDITIONAL_OUTCOME_SUBJECT_TOKEN_PATTERN_SOURCE})?)`;
...
const CONDITIONAL_OUTCOME_ACTION_COMPLEMENT_PATTERN_SOURCE = String.raw`(?:active|blue|brighter|closed|cooler|dimmer|eco|green|higher|inactive|locked|lower|off|on|open|red|unlocked|warmer|white|(?:at|by|to)\s+(?:[-+]?\d+(?:\.\d+)?|(?:(?:minus|negative)\s+)?${CONDITIONAL_OUTCOME_SPOKEN_SCALAR_VALUE_PATTERN_SOURCE})\s*(?:%|celsius|degrees?|fahrenheit|percent|°\s*(?:c|f))?)`;
const CONDITIONAL_OUTCOME_ACTION_TARGET_PATTERN_SOURCE = String.raw`(?:${AGGREGATE_DEVICE_CATEGORY_PATTERN_SOURCE}|${DEVICE_ACTION_TARGET_PATTERN.source}|${HOME_ENTITY_PATTERN.source}|${PLAUSIBLE_CUSTOM_HOME_TARGET_PATTERN.source}|${SCENE_TARGET_PATTERN.source})`;
...
const CONDITIONAL_OUTCOME_DETERMINED_REPORT_SUBJECT_PATTERN_SOURCE = String.raw`(?:he|i|it|she|they|we|you|(?:a|an|her|his|its|my|our|the|their|this|these|those|your)\s+${CONDITIONAL_OUTCOME_SUBJECT_TOKEN_PATTERN_SOURCE}(?:\s+${CONDITIONAL_OUTCOME_SUBJECT_TOKEN_PATTERN_SOURCE}){0,3})`;
...
const CONDITIONAL_OUTCOME_DIRECT_WHO_SUBJECT_PATTERN = new RegExp(
	String.raw`\b${CONDITIONAL_OUTCOME_ACTION_TARGET_PATTERN_SOURCE}\b(?:\s+${CONDITIONAL_OUTCOME_ACTION_COMPLEMENT_PATTERN_SOURCE})?(?:\s*,?\s+(?:and\s+)?then)?\s*$`,
	'u',
);
