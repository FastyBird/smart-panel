...
const CONDITIONAL_OUTCOME_NON_AGENT_SUBJECT_PATTERN_SOURCE = String.raw`(?:(?:a|an|her|his|its|my|our|the|their|this|these|those|your)\s+)?(?:[\p{Letter}\p{Number}'’-]+\s+){0,2}(?:${AGGREGATE_DEVICE_CATEGORY_PATTERN_SOURCE}|(?:alarm|camera|controller|detector|device|door|fan|heater|home|house|humidity|light|lock|power|room|router|sensor|switch|system|temperature|thermostat|window)s?)`;
...
const CONDITIONAL_OUTCOME_ZERO_RELATIVE_TARGET_PATTERN = new RegExp(
	String.raw`^${CONDITIONAL_OUTCOME_FIRST_ACTION_TARGET_PATTERN_SOURCE}\s+(?:${CONDITIONAL_OUTCOME_ACTION_COMPLEMENT_PATTERN_SOURCE}\s+)?${CONDITIONAL_OUTCOME_OBJECT_GAP_SUBJECT_PATTERN_SOURCE}\s+(?:keeps?|starts?|stops?)\s+${CONDITIONAL_OUTCOME_OBJECT_GAP_PREDICATE_PATTERN_SOURCE}(?:\s+${CONDITIONAL_OUTCOME_PREDICATE_ADJUNCT_PATTERN_SOURCE})?\s*\?\s*$`,
	'u',
);
