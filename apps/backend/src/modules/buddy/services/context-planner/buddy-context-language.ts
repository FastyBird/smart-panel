...
const CONDITIONAL_OUTCOME_OBJECT_GAP_INFINITIVE_PATTERN_SOURCE = String.raw`(?!${CONDITIONAL_OUTCOME_HUMAN_STATE_INFINITIVE_PATTERN_SOURCE}\b)[\p{Letter}'’-]+`;
...
const CONDITIONAL_OUTCOME_BARE_SUBJECT_COMMAND_PATTERN_SOURCE = String.raw`(?:${ACTION_SIGNAL_PATTERN_SOURCE}|also|ask|get|have|help|let|only|please|tell)`;
const CONDITIONAL_OUTCOME_SUBJECT_PREFIX_EXCLUSION_PATTERN_SOURCE = String.raw`(?!(?:albeit|although|as|because|even\s+(?:if|though)|since|so|that|though|to|where|whereas|which|who|whose)\b|${CONDITION_PATTERN.source}|(?:${COMPOUND_CONNECTOR_PATTERN_SOURCE})\b)`;
...
const CONDITIONAL_OUTCOME_ZERO_RELATIVE_TARGET_PATTERN = new RegExp(
	String.raw`^${CONDITIONAL_OUTCOME_FIRST_ACTION_TARGET_PATTERN_SOURCE}\s+(?:${CONDITIONAL_OUTCOME_ACTION_COMPLEMENT_PATTERN_SOURCE}\s+)?${CONDITIONAL_OUTCOME_OBJECT_GAP_SUBJECT_PATTERN_SOURCE}\s+(?:(?:keeps?|starts?|stops?)\s+${CONDITIONAL_OUTCOME_OBJECT_GAP_PREDICATE_PATTERN_SOURCE}(?:\s+${CONDITIONAL_OUTCOME_PREDICATE_ADJUNCT_PATTERN_SOURCE})?|${CONDITIONAL_OUTCOME_FINITE_OBJECT_GAP_PREDICATE_PATTERN_SOURCE})\s*\?\s*$`,
	'u',
);
...
function hasConditionalOutcomeRelativePrefix(prefix: string): boolean {
	...
	if (CONDITIONAL_OUTCOME_WHICH_OBJECT_RELATIVE_PREFIX_PATTERN.test(prefix)) return true;
