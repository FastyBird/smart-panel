...
const CONDITIONAL_OUTCOME_SUBJECT_PREFIX_EXCLUSION_PATTERN_SOURCE = String.raw`(?!(?:albeit|although|as|because|even\s+(?:if|though)|since|so|that|though|to|where|whereas|wherein|wherever|whether|which|whilst|who|whose)\b|${CONDITION_PATTERN.source}|(?:${COMPOUND_CONNECTOR_PATTERN_SOURCE})\b)`;
