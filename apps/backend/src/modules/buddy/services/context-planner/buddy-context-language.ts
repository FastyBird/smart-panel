...
const CONDITIONAL_OUTCOME_SUBJECT_PATTERN_SOURCE = String.raw`(?!(?:because|so)\b|${CONDITION_PATTERN.source})(?!(?:(?:${COMPOUND_CONNECTOR_PATTERN_SOURCE})\s+)?(?:${ACTION_SIGNAL_PATTERN_SOURCE}|help|let)\b)(?:(?:a|an|my|our|the|their|this|these|those|your)\s+)?[\p{Letter}\p{Number}'’-]+(?:\s+[\p{Letter}\p{Number}'’-]+){0,3}`;
