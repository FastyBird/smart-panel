...
export const RELATIVE_REFERENCE_PRONOUN_PATTERN = new RegExp(
	String.raw`\b(?:that|which)\s+(?:${AGGREGATE_STATE_MODIFIER_PATTERN_SOURCE}\s+)?${STATE_LINKING_VERB_PATTERN_SOURCE}\b(?=\s+(?:${PROGRESSIVE_STATE_LINKER_PATTERN_SOURCE}\s+)?${AGGREGATE_STATE_VALUE_PATTERN_SOURCE}\b)`,
	'gu',
);
