...
export const AGGREGATE_STATE_COORDINATION_PATTERN = new RegExp(
	String.raw`\b${AGGREGATE_READ_QUANTIFIER_PATTERN_SOURCE}\b[^?!,.;]{0,80}\b${WRAPPED_AGGREGATE_HOME_TARGET_PATTERN_SOURCE}\b[^?!,.;]{0,40}\b${AGGREGATE_STATE_VALUE_PATTERN_SOURCE}\s+${AGGREGATE_READ_COORDINATED_STATE_PATTERN_SOURCE}\b(?=\s*(?:(?:${AGGREGATE_READ_TRAILING_ADJUNCT_PATTERN_SOURCE})\s*)*(?:(?:${AGGREGATE_READ_LOCAL_QUALIFIER_PATTERN_SOURCE})\s*)?[?!,.;]?$)`,
	'gu',
);
...
const AGGREGATE_READ_SENTENCE_BOUNDARY_PATTERN_SOURCE = String.raw`(?=\s*(?:(?:${AGGREGATE_READ_COORDINATED_STATE_PATTERN_SOURCE})\s*)?(?:(?:${AGGREGATE_READ_RELATIVE_TAIL_PATTERN_SOURCE})\s*)?(?:(?:,\s*)?(?:${AGGREGATE_READ_TRAILING_ADJUNCT_PATTERN_SOURCE})\s*)*[?!,.;]?$)`;
