/**
 * Transformer Types and Interfaces for Shelly NG
 *
 * Defines the transformer system for converting values between
 * Shelly devices and Smart Panel formats bidirectionally.
 */
...
export interface ScaleTransformerDefinition extends TransformerDefinition {
	type: 'scale';
	input_range: [number, number]; // Shelly range [min, max]
...
export interface MapTransformerDefinition extends TransformerDefinition {
	type: 'map';
	read?: Record<string, unknown>; // Shelly value -> Panel value
	write?: Record<string, unknown>; // Panel value -> Shelly value
	bidirectional?: Record<string, unknown>; // Auto-creates inverse for write
	/** Formula to use for write when no map entry is found */
...
export interface FormulaTransformerDefinition extends TransformerDefinition {
	type: 'formula';
	read?: string; // Expression for read (Shelly -> Panel)
	write?: string; // Expression for write (Panel -> Shelly)
}

/**
 * Boolean transformer - true/false conversions
 * Automatically invertible
 */
export interface BooleanTransformerDefinition extends TransformerDefinition {
	type: 'boolean';
	true_value: unknown; // Shelly value representing true
	false_value: unknown; // Shelly value representing false
...
export interface ITransformer {
	/**
	 * Transform value from Shelly format to Smart Panel format
	 * @param value - Value from Shelly device
	...
	/**
	 * Transform value from Smart Panel format to Shelly format
	 * @param value - Value from Smart Panel
	 * @param context - Optional transformation context
	 * @returns Transformed value for Shelly device
	 */
