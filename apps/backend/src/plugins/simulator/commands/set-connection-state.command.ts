...
export class SetConnectionStateCommand extends CommandRunner {
	private readonly logger = createExtensionLogger(SIMULATOR_PLUGIN_NAME, 'SetConnectionStateCommand');
	...
	async run(_passedParams: string[], options?: SetConnectionStateOptions): Promise<void> {
		...
		const allDevices = await this.devicesService.findAll<SimulatorDeviceEntity>(SIMULATOR_TYPE);
