...
import { SpaceCategoryTemplate, SpaceRoomCategory, SpaceZoneCategory } from '../../modules/spaces/spaces.constants';
...
export const SPACES_HOME_CONTROL_PLUGIN_API_TAG_NAME = 'Spaces home control plugin';

export const SPACES_HOME_CONTROL_PLUGIN_API_TAG_DESCRIPTION =
	'Endpoints for the home-control space type. Contributes the built-in Room and Zone space types along with their lighting, climate, covers, sensor, and media role domains, intent catalog, suggestions, and undo history.';

/**
 * Domain event types emitted by spaces-home-control domain services (lighting,
 * climate, covers, sensors, media, suggestions). These were originally all
 * co-located with the space lifecycle events in the core `EventType` enum
 * but have been split out as part of Phase 3 plugin extraction — the string
 * values are unchanged so runtime WebSocket payloads and subscribers keep
 * working. Core retains `EventType.SPACE_CREATED/UPDATED/DELETED` in
 * `modules/spaces/spaces.constants.ts`.
 */
