...
export class OpenWeatherMapForecastListItemDto {
	@Expose()
	@IsInt()
	dt: number;
...
export class OpenWeatherMapForecastResponseDto {
	@Expose()
	@IsString()
	cod: string;
	...
	@IsArray()
	@ValidateNested({ each: true })
	@Type(() => OpenWeatherMapForecastListItemDto)
	list: OpenWeatherMapForecastListItemDto[];
