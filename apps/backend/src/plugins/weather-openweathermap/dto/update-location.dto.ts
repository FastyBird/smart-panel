...
import { OpenWeatherMapLocationType } from '../weather-openweathermap.constants';

interface LocationTypeContext {
	locationType: OpenWeatherMapLocationType;
}

@ApiSchema({ name: 'WeatherOpenweathermapPluginUpdateLocation' })
export class UpdateOpenWeatherMapLocationDto extends UpdateLocationDto {
	@ApiPropertyOptional({
