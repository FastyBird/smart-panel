...
/**
 * Registers the `send-test` diagnostic action for the webhook plugin - the only UI the channel
 * needs beyond its config form, since the Actions tab already renders it.
 */
@Injectable()
export class WebhookActionsService implements OnModuleInit {
	...
	constructor(
		...
		private readonly channel: WebhookChannelPlatform,
	) {}

	onModuleInit(): void {
		this.actionRegistry.register(NOTIFICATIONS_WEBHOOK_PLUGIN_NAME, this.createSendTestAction());
	}
	...
	private createSendTestAction(): IExtensionAction {
		return {
			...
			description: 'Sends a sample notification through this webhook to verify it is configured correctly.',
			icon: 'mdi:webhook',
			...
			execute: async (): Promise<IActionResult> => {
				...
				try {
					...
					return { success: true, message: 'Test notification sent to the configured webhook.' };
				} catch (error) {
					const message = sanitizeErrorMessage(this.toErrorMessage(error));

					this.logger.warn(
						`Test notification failed for channel type=${NOTIFICATIONS_WEBHOOK_PLUGIN_NAME}: ${message}`,
					);
	...
	private buildSampleNotification(): NotificationEntity {
		...
		return {
			id: randomUUID(),
			source: NOTIFICATIONS_WEBHOOK_PLUGIN_NAME,
			kind: NotificationKind.EVENT,
			key: null,
			severity: NotificationSeverity.INFO,
			title: 'Test notification from Smart Panel',
			message: 'This is a test notification. If you can see this, the webhook channel is configured correctly.',
			actions: [],
			data: null,
			persistent: false,
			occurrences: 1,
