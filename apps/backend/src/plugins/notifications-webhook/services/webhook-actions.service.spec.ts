...
import { WebhookActionsService } from './webhook-actions.service';
...
describe('WebhookActionsService', () => {
	...
	const getRegisteredAction = (): RegisteredAction => {
		...
		expect(actionRegistry.register).toHaveBeenCalledWith(NOTIFICATIONS_WEBHOOK_PLUGIN_NAME, expect.any(Object));
	...
	describe('onModuleInit', () => {
		it('registers a single send-test action for the webhook plugin', () => {
		...
		it('returns a sanitized failure message when the channel throws', async () => {
			channel.send.mockRejectedValue(
				new ChannelDeliveryError(
					'Channel responded with HTTP 401 for https://example.com/hooks/secret-path',
					false,
					401,
				),
			);
