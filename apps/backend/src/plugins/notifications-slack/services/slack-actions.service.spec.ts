...
import { SlackActionsService } from './slack-actions.service';
...
describe('SlackActionsService', () => {
	...
	const getRegisteredAction = (): RegisteredAction => {
		...
		expect(actionRegistry.register).toHaveBeenCalledWith(NOTIFICATIONS_SLACK_PLUGIN_NAME, expect.any(Object));
	...
	describe('onModuleInit', () => {
		it('registers a single send-test action for the slack plugin', () => {
		...
		it('returns a sanitized failure message when the channel throws', async () => {
			channel.send.mockRejectedValue(
				new ChannelDeliveryError(
					'Channel responded with HTTP 401 for https://hooks.slack.com/services/T0/B0/XYZ',
					false,
					401,
				),
			);
