...
export class BuddyOpenaiPlugin implements OnModuleInit {
	...
	onModuleInit() {
		...
		this.extensionsService.registerPluginMetadata({
			type: BUDDY_OPENAI_PLUGIN_NAME,
			name: 'Buddy OpenAI',
			description: 'LLM provider for Buddy module using OpenAI API',
			author: 'FastyBird',
			readme: `# Buddy OpenAI Provider

LLM provider plugin for the Buddy module using the OpenAI API.
...
4. Set the buddy module \`provider\` to \`${BUDDY_OPENAI_PLUGIN_NAME}\`
