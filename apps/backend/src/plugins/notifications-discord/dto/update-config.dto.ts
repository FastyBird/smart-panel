...
import { IsValidDiscordWebhookUrl, MAX_DISCORD_WEBHOOK_URL_LENGTH } from '../validators/discord-webhook-url.validator';

@ApiSchema({ name: 'NotificationsDiscordPluginUpdateConfig' })
export class UpdateNotificationsDiscordConfigDto extends UpdatePluginConfigDto {
	@ApiProperty({
	...
	type: typeof NOTIFICATIONS_DISCORD_PLUGIN_NAME;
	...
	@ApiPropertyOptional({
		description: 'Replacement Discord webhook URL. Omit to preserve the stored URL or send null to clear it.',
		name: 'webhook_url',
		type: 'string',
		nullable: true,
	})
	...
	@IsValidDiscordWebhookUrl({
		message: `[{"field":"webhook_url","reason":"Webhook URL must start with https:// and be at most ${MAX_DISCORD_WEBHOOK_URL_LENGTH} characters without embedded credentials."}]`,
	})
	...
	@Expose()
	@IsOptional()
	@IsString({ message: '[{"field":"username","reason":"Username must be a string."}]' })
