...
class ProjectorDeviceDetail extends StatefulWidget {
	final ProjectorDeviceView _device;
	final VoidCallback? onBack;
	final DeviceDetailConfig? config;

	const ProjectorDeviceDetail({
		super.key,
		required ProjectorDeviceView device,
		this.onBack,
		this.config,
	}) : _device = device;

	@override
	State<ProjectorDeviceDetail> createState() => _ProjectorDeviceDetailState();
}
...
class _ProjectorDeviceDetailState extends State<ProjectorDeviceDetail> {
	...
	void initState() {
		...
		try {
			...
			if (kDebugMode) {
				debugPrint('[ProjectorDeviceDetail] Failed to get DeviceControlStateService: $e');
			}
		}
	}
	...
	void _checkConvergence() {
		...
		final brightnessProp = _device.projectorChannel.brightnessProp;
		if (brightnessProp != null) {
			controlState.checkPropertyConvergence(
				_device.id,
				_device.projectorChannel.id,
				brightnessProp.id,
				_device.projectorBrightness,
				tolerance: 1.0,
			);
		}
	...
	ProjectorDeviceView get _device {
		final updated = _devicesService.getDevice(widget._device.id);
		if (updated is ProjectorDeviceView) {
			return updated;
		}
	...
	void _togglePower() {
		final propId = _device.projectorChannel.onProp.id;

		_devicesService.setPropertyValueWithContext(
			deviceId: _device.id,
			channelId: _device.projectorChannel.id,
			propertyId: propId,
			value: !_device.isProjectorOn,
		);
	}
	...
	void _sendRemoteKey(ProjectorRemoteKeyValue key) {
		final prop = _device.projectorChannel.remoteKeyProp;
		if (prop == null) return;
	...
	int get _effectiveBrightness {
		...
		if (controlState != null && prop != null &&
			controlState.isLocked(_device.id, _device.projectorChannel.id, prop.id)) {
			final desired = controlState.getDesiredValue(_device.id, _device.projectorChannel.id, prop.id);
			if (desired is num) return desired.toInt();
		}

		return _device.projectorBrightness;
	}
	...
	void _showSettingsSheet() {
		...
		// In landscape, brightness is already in the right column
