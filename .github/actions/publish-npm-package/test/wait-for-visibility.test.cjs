...
test('waits past the previous 90-probe cutoff', async () => {
	const testRoot = await mkdtemp(join(tmpdir(), 'publish-visibility-'));
	await mkdir(join(testRoot, 'bin'));
	await writeFile(join(testRoot, 'attempts'), '0');
	await writeFile(
...
`,
	...
	assert.equal(result.code, 0);
	assert.match(result.output, /visible on https:\/\/registry\.example/);
	assert.equal(await readFile(join(testRoot, 'attempts'), 'utf8'), '91\n');
	await rm(testRoot, { recursive: true, force: true });
});
...
`,
	...
	assert.match(result.output, /after 240 attempts/);
	assert.equal(await readFile(join(testRoot, 'attempts'), 'utf8'), '240\n');
	await rm(testRoot, { recursive: true, force: true });
});
