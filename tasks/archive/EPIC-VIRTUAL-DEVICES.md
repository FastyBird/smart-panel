...
- **The six closed-loop categories** stay blocked, as [the design's v1 boundary](../../docs/superpowers/specs/2026-07-31-virtual-devices-design.md#v1-category-boundary) sets out. Unblocking them needs a control loop with hysteresis and minimum cycle protection, plus user-settable setpoints as owned properties — the schema already accommodates the latter. It starts with a design document rather than a task, for the reason the roadmap gives: a control loop touches safety, concurrency and the platform's command path, and the constraints already surfaced for it are recorded in the discussion on PR #645.
- **Per-channel energy attribution**, so a split device's consumption lands in its own room. Filed as [`BUG-ENERGY-VIRTUAL-ROOM-ATTRIBUTION`](../bugs/BUG-ENERGY-VIRTUAL-ROOM-ATTRIBUTION.md) rather than left as the "pre-existing limitation" this epic first called it: the reproduction shows a split device reporting *zero* in every room it was split into, which the split itself creates the expectation of.
