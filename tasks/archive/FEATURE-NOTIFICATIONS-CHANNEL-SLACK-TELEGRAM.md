...
- [x] `notifications-slack` config model exposes `webhook_url` (write-only secret) and `min_severity`; the DTO
      rejects a `webhook_url` that does not start with `https://`.
- [x] `notifications-telegram` config model exposes `bot_token` (write-only secret), `chat_id` (string) and
      `min_severity`.
- [x] The Slack channel's `send()` posts `{ text: title, attachments: [{ color, title, text: message, footer
...
- [x] The Telegram channel's `send()` calls `POST https://api.telegram.org/bot<token>/sendMessage` with `{
...
- [x] The Telegram channel HTML-escapes `<`, `>` and `&` in both `title` and `message` before building
      `text`.
- [x] The Telegram channel never logs the bot token; log output includes only `api.telegram.org` and the
      response status.
- [x] Each plugin registers its own `send-test` extension action identical in shape to
...
- [x] Both plugins' secrets (`webhook_url` for Slack, `bot_token` for Telegram) are redacted through
...
- [x] A Telegram-specific test proves the HTML-escaping of `<`, `>` and `&` in the outgoing text.
- [x] A Telegram-specific test proves an `ok: false` JSON reply (with an HTTP 200 status) causes `send()` to
      throw a non-retryable `ChannelDeliveryError`.
- [x] A Slack config DTO test proves a `webhook_url` that does not start with `https://` is rejected.
- [x] Channel specs (mocked `fetch`) assert request shape for both plugins, matching the coverage
      `FEATURE-NOTIFICATIONS-CHANNEL-WEBHOOK-DISCORD` established.
- [x] `cd apps/backend && npx jest src/plugins/notifications-slack src/plugins/notifications-telegram src/plugins/plugin-secret-removal.spec.ts`
      passes.
