...
**Status as of 2026-04-23 re-audit** — all seven phases have merged PRs, but Phase 3 landed as scaffolding only: the `spaces-home-control` plugin is registered and dispatched against, yet almost none of the domain code was physically relocated. Phase 4–7 all sit on top of that scaffold, so they work at runtime, but `modules/spaces/` still owns ~80 files that conceptually belong to the plugin. See "Outstanding work" below for the remaining relocation.
...
- 🟡 **Phase 3a — SKELETON ONLY** — merged, but the 12-step incremental relocation in "Phase 3a incremental commit roadmap" below never progressed past step 1. Current state of `apps/backend/src/plugins/spaces-home-control/`: `spaces-home-control.plugin.ts`, `spaces-home-control.constants.ts`, `spaces-home-control.openapi.ts`, `dto/update-config.dto.ts`, `models/config.model.ts`, `services/home-control-home-page.resolver.ts`. That's it. Every Room/Zone entity, all 6 role entities, 29 domain services, all domain DTOs, the 2398-line `spaces.controller.ts`, the 3 domain state listeners, the intent spec (`spec/`), and both domain validators (`at-least-one-setpoint.validator.ts`, `setpoint-order-constraint.validator.ts`) still live under `apps/backend/src/modules/spaces/`. The plugin is registered in `app.module.ts` + `RouterModule` and shows up in the extensions list (after the `PluginsTypeMapperService.registerMapping` fix on 2026-04-23), but it's an empty host. **Remaining:** steps 2–13 of the roadmap below.
- 🟡 **Phase 3b — SKELETON ONLY** — merged, but equivalent to Phase 3a's status. `apps/admin/src/plugins/spaces-home-control/components/components.ts` is a single re-export barrel that proxies `SpaceAddForm`, `SpaceDetail`, `SpaceEditForm` from `../../../modules/spaces/components/components`. All 67 Vue components and 20+ composables (`useSpace*`, `useSpaceAddForm`, `useSpaceCategories`, `useSpaceLightingState`, `useSpaceClimateState`, `useSpaceCoversState`, `useSpaceDevices`, `useSpaceDisplays`, `useSpaceIntents`, `useSpaceMedia`, `useSpaceScenes`, etc.) still live under `apps/admin/src/modules/spaces/`. The `view-space-edit.vue` shell + `useSpacesPlugins` dispatch work correctly (home-control's elements resolve to the re-exported components), so the plugin system functions — it just hasn't absorbed the code it's supposed to own. **Remaining:** move the Vue SFCs + composables + Pinia pieces that are domain-specific (lighting roles panel, climate state table, intent-catalog viewer, etc.) into `plugins/spaces-home-control/`; leave only generic space CRUD in `modules/spaces/`.
...
The feature is functional end-to-end, but the following items are on the plan and not yet done. A future agent should tackle these in order; each is independently ship-able.

#### 1. Phase 3a body — backend code relocation (LARGE)

The plugin host exists; the code that belongs inside it is still in `modules/spaces/`. Steps 2–13 of the "Phase 3a incremental commit roadmap" (below in the Phase 3 section) describe the ~80-file move in 12 focused commits. Concrete scope:

- `git mv apps/backend/src/modules/spaces/spec/ → apps/backend/src/plugins/spaces-home-control/spec/` (intent loader + YAML catalog).
- `git mv` 6 role entities (`space-{lighting,climate,covers,sensor}-role.entity.ts`, `space-media-activity-binding.entity.ts`, `space-active-media-activity.entity.ts`). Move `spacesTypeMapper.registerMapping` / `spaceRolesTypeMapper.registerMapping` calls to the plugin's `onModuleInit`.
- `git mv` `room-space.entity.ts`, `zone-space.entity.ts`; abstract `SpaceEntity` stays in core.
- `git mv` domain DTOs (`climate-intent.dto.ts`, `climate-role.dto.ts`, `covers-intent.dto.ts`, `covers-role.dto.ts`, `lighting-intent.dto.ts`, `lighting-role.dto.ts`, `media-activity-binding.dto.ts`, `sensor-role.dto.ts`, `suggestion.dto.ts`). Keep `create-space.dto.ts`, `update-space.dto.ts`, `bulk-assign.dto.ts`, `status-widget.dto.ts`, `update-config.dto.ts` in core.
- `git mv` both domain validators (`at-least-one-setpoint.validator.ts`, `setpoint-order-constraint.validator.ts`). Keep `space-category-constraint.validator.ts` in core.
- `git mv` 3 domain state listeners (`space-lighting-state.listener.ts`, `space-climate-state.listener.ts`, `space-sensor-state.listener.ts`). Keep `space-activity.listener.ts` + `websocket-exchange.listener.ts` in core.
- `git mv` ~24 domain services out of the 29 currently in `modules/spaces/services/`. Core keeps: `spaces.service.ts`, `spaces-type-mapper.service.ts`, `space-roles-type-mapper.service.ts`, `space-create-builder-registry.service.ts`, `space-relations-loader-registry.service.ts`, `module-reset.service.ts`. Everything else (lighting/climate/covers/sensor role + state, intent services, context-snapshot, suggestion, suggestion-heartbeat, derived-media-endpoint, media-capability, media-activity + binding, lighting-tool) moves.
- Split `spaces.controller.ts` (2398 lines): core keeps generic CRUD + `children/parent/devices/displays/assign` + `GET /zones`; move the ~50 domain endpoints into per-domain plugin controllers (`lighting`, `climate`, `covers`, `sensors`, `media`, `intent-catalog`, `suggestions`, `undo`). Preserve URLs (`/spaces/:id/lighting` etc.) for admin/panel compatibility.
- Split `spaces-response.model.ts` — generic `SpaceResponseModel`/`SpacesResponseModel` stay in core; domain state + intent-catalog response models move to plugin.
- Split `spaces.constants.ts` — keep `SpaceType`, `SpaceRoleType`, `SPACE_CREATED/UPDATED/DELETED`, `SuggestionFeedback`; move domain event types (`LIGHT_TARGET_*`, `CLIMATE_TARGET_*`, `COVERS_TARGET_*`, `SENSOR_TARGET_*`, `MEDIA_*`, `SUGGESTION_*`, `UNDO_*`) to plugin constants.
- Cross-plugin importer fixups: `plugins/simulator/*` currently imports role services + `SpaceType` directly from `modules/spaces/services/*` — redirect to plugin paths.
- Add `1000000000007-SpacesHomeControlPlugin.ts` seed migration (no DDL; inserts plugin row in extensions table).
- OpenAPI regen should produce **no diff** — every change is internal relocation, schemas stay byte-for-byte identical.

#### 2. Phase 3b body — admin code relocation

Mirror 3a's relocation on the admin side. Every domain component + composable that belongs to home-control should live under `apps/admin/src/plugins/spaces-home-control/`:

- Components: everything domain-specific currently in `apps/admin/src/modules/spaces/components/` (roles panels for lighting/climate/covers/sensor, state tables, intent-catalog viewer, suggestion list, undo history, media-activity editor — the bulk of the 67 files). Keep generic CRUD pieces (`space-add-form.vue`, `space-edit-form.vue`, `select-space-plugin.vue`, `space-select.vue`, `spaces-table.vue`, `spaces-list.vue`, `spaces-cards.vue`) in core.
- Composables: `useSpaceLightingState`, `useSpaceClimateState`, `useSpaceCoversState`, `useSpaceIntents`, `useSpaceMedia`, `useSpaceScenes`, `useSpaceDevices`, `useSpaceDisplays` — domain-specific, relocate. Keep `useSpace`, `useSpaceAddForm`, `useSpaceEditForm`, `useSpaceCategories`, `useSpacesPlugins` in core.
- Replace the `components.ts` re-export barrel with real imports from the relocated files.
- Update admin locales: split `spacesModule.*` keys into `spacesHomeControlPlugin.*` for the domain-specific UI strings. Preserve English fallback.
- Admin type-check + lint + vitest unit suite must stay green.

#### 3. Signage `spaceAddForm` component

The new plugin-picker flow (Phase 6 follow-up) only surfaces types whose plugin contributes a `spaceAddForm`. Signage registers a `spaceEditForm` but not an add form, so the user can't create an info-panel space from admin today. Minimum viable fix:

- New `apps/admin/src/plugins/spaces-signage-info-panel/components/signage-info-panel-space-add-form.vue`. Model: `name`, `description`, `layout`, `showClock`, `showWeather`, `showAnnouncements`, `showFeed`, `weatherLocationId`, `feedUrl` (mirrors the edit form minus the announcements section — announcements get added after the space exists).
- POST to `/modules/spaces/spaces` with the signage subtype payload. Emit `saved(space)` like the home-control add form.
- Register it on the plugin's element: `components: { spaceAddForm: SignageInfoPanelSpaceAddForm, spaceEditForm: SignageInfoPanelSpaceEditForm }`.
- Signage now appears in the picker; picking it dispatches the new add form.

#### 4. Cleanup

- The `IRelationLoader` interface is duplicated verbatim in `modules/spaces/entities/space.relations.ts` and `modules/dashboard/entities/dashboard.relations.ts`. Phase 3a is the natural moment to consolidate into `apps/backend/src/common/plugin-relations/` (deferred from Phase 1a with a PR reply explaining the timing).
- `useSpacesPlugins.getByName` (and the same-name methods on the dashboard's `usePagesPlugins`, `useTilesPlugins`, `useDataSourcesPlugins` composables) actually searches `plugin.type`, not `plugin.name`. Bugbot has flagged this multiple times. A cross-module rename to `getByType` / `getByPluginType` + a follow-up PR covering all four composables is cleaner than fixing spaces alone.
