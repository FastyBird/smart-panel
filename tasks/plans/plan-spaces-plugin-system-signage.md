...
- 🟡 **Phase 3a** — In progress on branch `claude/spaces-plugin-signage-TRG2E`. Backend plugin extraction. Skeleton committed as `f2dcfc5` (empty plugin registered in `app.module.ts` + `RouterModule`, extension metadata registered, no providers/controllers yet). Remaining work broken into 12 sequential commits — see "Phase 3a incremental commit roadmap" in the Phase 3 section below. Next step: move `spec/` (intent spec loader + YAML catalog) into the plugin.
- ✅ **Phase 3b, 3c** — Merged via PRs #580 → #583. Admin + panel skeletons for `spaces-home-control` landed; dispatch infra + locales in place, `view-space-edit.vue` shells through `useSpacesPlugins`. Panel widgets relocated under `plugins/spaces-home-control/` (registration still goes via legacy `DisplayRole` switch — that rewires in Phase 5).
