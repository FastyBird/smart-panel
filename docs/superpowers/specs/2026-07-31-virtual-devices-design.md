...
5. **What serialises, and around what.** Per *device*, not per loop — a heater and a cooler evaluating concurrently each see the other inactive and each energise. `DeviceStructureLockService` is the existing shape to borrow from, not necessarily the instance. Note this only serialises what goes through it, which is what makes 4 a prerequisite rather than a detail.
    ...
    Either way, the question a lifetime answers is not "how long does the record live" but "what is assumed when it runs out", and there is only one safe answer: an unconfirmed *energising* command stays interlock-blocking until the actuator is read back and confirmed off. A command that timed out can still complete late — the platform was slow, not silent — so expiring the record and treating the output as off is how both actuators end up on with every stated rule obeyed. Unknown is not off. A timeout should drive reconciliation against the actuator, and only its answer should release the interlock.
