...
**Deliverable.** A design spec beside this one, answering 1–4 with file-level references the way this section does, then one task per phase of the scope order. Not a single task: the first phase changes the schema and the platform command path, and reviewing that inside a feature that also ships a control loop is how both get less attention than they need.
