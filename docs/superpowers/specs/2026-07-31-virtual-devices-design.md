...
2. **What `status` is.** The requirement is on the *channel*, not the category, which is the same distinction the interlock turns on: `heater` and `cooler` each require a boolean `status(ro)` no source device supplies, so every phase owes an answer — including `thermostat`, whose selected heater or cooler channel requires one exactly as `heating_unit`'s does. The `humidifier` and `dehumidifier` channels type theirs as an optional `enum` instead, so a synthesized boolean fails validation there rather than satisfying it. Either take it as another mapping, or synthesize it — and either way it has to be **feedback from the claimed actuator**, never anything else. Synthesized, that means the actuator's confirmed state and not the loop's decision: a controller recording its intention reports the unit idle while the relay is still energised after a failed release, and suppresses the retry. Mapped, it means the same discipline applied to a mapping the operator chooses, and the wizard cannot be left to accept any compatible read-only boolean — mappings are deliberately cross-device and reusable, so an unrelated property satisfies the slot's type and permissions perfectly while reporting a unit that is not this one. `SpaceClimateStateService.detectClimateModeAndActivity()` consumes whatever lands there as the unit's actual activity, so both routes end in the same place: `status` is a claim about the relay, and only the relay can make it. The choice changes what a "ready" device means in the wizard.
3. **How a command reaches the setpoint, and what it is allowed to leave behind.** `VirtualDevicePlatform.processBatch()` refuses every `LOCAL` property outright, and the panel and climate intents go through it rather than PATCHing. Relaxing `assertOwnedPropertyNotWritable` alone leaves the ordinary climate controls unable to move either the setpoint or the enable.

    Reachability is only half of it. `SetpointOrderConstraintValidator` checks `heating < cooling` **only when both arrive together** — "if either is not provided, no cross-field validation needed" — and `executeSetpointSetIntent()` writes only the bound it was given, leaving its counterpart as it was. Two successive single-bound updates therefore persist a crossed pair: cooling at 24, then heating at 25. That is inert in v1, where nothing acts on a setpoint. Under a loop it is an instruction to hold the room above 25 and below 24 at once, and the answer is changeover oscillation — heat, cross the cooling threshold, cool, cross the heating threshold — which minimum-cycle protection then masks as slow cycling rather than reporting as a contradictory configuration.

    So the controller's command path validates the **merged persisted pair**, not the payload it was handed, and the ordering rule is not sufficient either: a valid `heating 22.0 / cooling 22.2` oscillates just as surely under ±0.3 hysteresis. The gap between the two has to be at least the hysteresis band the loop actually uses, which makes the deadband a property of the pair rather than of either setpoint, and something the wizard and the intent path both have to be able to state.
4. **What else may write to a claimed actuator.** Exclusivity against a second *controller* is not exclusivity against a second *writer*, and the relay has other ones. This design deliberately lets one source property feed several virtual devices, so nothing above stops a virtual switch or outlet from projecting the same relay's `on` and exposing it as an ordinary toggle; and a stored scene action commands the physical property directly — `@ValidateDeviceNotHidden` refuses a *new* reference to a hidden device and leaves existing ones executing (§3a.14). Both bypass the loop's serialisation entirely: either can energise a relay the controller believes it released, or start a cooler while its opposed heater is running, which is the interlock defeated from outside the mechanism enforcing it. So the claim has to either **withhold the property from every other writable reference and command path**, or those paths have to be **mediated** — routed through whatever owns the actuator — and the choice is not obvious: withholding breaks the "one source, many virtual devices" principle for exactly one kind of property, while mediating means the controller becomes a command path other modules have to know about.
    ...
    Both of those bound only what *this* application does, and that is not the whole set of writers. A relay imported from Home Assistant is still commandable by HA automations and by the HA UI — the plugin subscribes to `state_changed` and learns about the toggle after it has happened (`home-assistant.ws.service.ts:606`) — and the same is true of a Shelly running its own schedule, or of a physical wall switch wired across the relay. The loop does react: an actuator value event is already one of its triggers, so it will correct the state. What it cannot do is prevent the window in between, and for two opposed actuators that window is a heater and a cooler running together.

    So the design owes an **upstream-writer boundary** as well, and the honest options are narrower than the in-process ones: a hardware interlock, so the contradiction is impossible rather than merely corrected; exclusive upstream control, which the design can require and document but cannot enforce; or refusing the opposed-actuator configurations outright where neither can be guaranteed.

    And there is a writer the ownership question does not cover at all: **nobody**. A relay that has confirmed ON stays on until something turns it off, and if the backend crashes, loses power or sits partitioned from the device, nothing does — the periodic tick cannot run, and the restart rules above only help if the service comes back. Exclusive ownership makes this *worse* rather than better, since it is precisely the arrangement in which no other writer will intervene either. A control loop cannot be the only thing between a heating element and an indefinite ON.

    So the design owes an answer that lives outside this process: an actuator-local maximum-on or watchdog timeout, fail-closed hardware, or refusing configurations where neither is available. Where the platform supports it, the shape to prefer is a **dead-man's switch** — a relay with its own auto-off timer that the loop re-arms while it wants the output, so the controller disappearing releases the actuator instead of stranding it. That inverts the failure mode rather than shortening it, and it is the only one of the three that does.

    It also has a limit that has to be said in the same breath, because it is easy to mistake for a guarantee: a watchdog fed by the thing it watches is not a watchdog. A sensor stuck low while still publishing keeps the freshness check happy, so the loop goes on wanting heat and goes on re-arming, and the timer never fires — a heater or a water heater energised indefinitely while every rule here is obeyed. Whatever bounds the output has to be **underivable from the same sensor decision**: an absolute maximum-on the loop cannot renew, or an independent high-limit measurement, which is what a real water heater's thermal cut-out is. A configuration with neither is one to refuse rather than to run.

    Those three are the *opposed-pair* answers, and they are not the whole obligation: a single actuator needs an ownership answer too. An upstream automation or a wall switch that energises a lone heater after the user disabled it heats a room nobody asked to heat, and the loop learns only after actuation — its corrective release can fail or arrive late. It also bypasses hysteresis and minimum-cycle protection entirely, and minimum-cycle exists to protect the hardware rather than the comfort, so "it costs a cycle" understates it. So: exclusive upstream ownership, or an equivalent fail-safe, for **every** actuator; hardware interlocking or refusal on top, for opposed pairs.
5. **What minimum-cycle protection may delay, and what it may not.** It exists to protect the hardware from short cycling, and applied indiscriminately it delays the one thing that must never wait: an explicit OFF from the user, or a release demanded by stale or faulted input, is not a cycle to be smoothed. An implementation can satisfy every stated cycle rule while continuing to heat after somebody said stop. So the rule has a direction — cycle timing gates *energising* and non-safety changeovers only; releases are dispatched immediately and retried until confirmed.
6. **What serialises, and around what.** Per *device*, not per loop — a heater and a cooler evaluating concurrently each see the other inactive and each energise. `DeviceStructureLockService` is the existing shape to borrow from, not necessarily the instance. Note this only serialises what goes through it, which is what makes 4 a prerequisite rather than a detail.

    Serialising the *evaluation* is not enough where the platform acknowledges asynchronously, which several do. One evaluation dispatches `heater=on` and releases; the confirming value event has not arrived; the next trigger — a sensed value, the periodic tick — takes the lock, reads the heater as still off, and energises the cooler. Both outputs on, with the stated serialisation satisfied throughout. And the invariant in 2 is what makes it invisible: committing state on confirmation is right for *reporting*, and it means an in-flight command exists nowhere the next evaluation can see it.

    So the boundary is confirmation, not dispatch, and there are two ways to hold it: keep the serialisation through the round trip, which needs a policy for an actuator that never confirms or the device stops evaluating entirely; or represent outstanding commands as interlock state, so a pending `heater=on` counts as active for the interlock while `status` still waits for the actuator. The second keeps 2 intact and is the smaller change to reporting.

    Either way, the question a lifetime answers is not "how long does the record live" but "what is assumed when it runs out", and there is only one safe answer: an unconfirmed *energising* command stays interlock-blocking. A command that timed out can still complete late — the platform was slow, not silent — so expiring the record and treating the output as off is how both actuators end up on with every stated rule obeyed. Unknown is not off.

    What releases it needs care too, and reading the actuator back is not enough on its own: a readback can happen while the original `heater=on` is still queued upstream, and answering "off" would release the interlock in front of a command that then lands. What proves the output is safe is that the earlier command can no longer take effect — a cancellation the platform honours, ordering or correlation that shows it drained, or an explicit *off* whose confirmation is known to have arrived after it. A bare observation of "off" proves only that it had not arrived yet.

    And none of this survives a restart by itself. A process that dies between dispatch and confirmation takes the lock or the pending record with it, and a fresh start reads the heater as off, energises the cooler, and receives the delayed command afterwards — the same failure, arrived at through a deployment. Outstanding energising commands have to be durable, or evaluation has to stay blocked until startup reconciliation establishes that nothing from before the restart is still in flight. This is the same requirement the cycle state above already carries, for the same reason: protection kept only in memory is no protection.
...
**Deliverable.** A design spec beside this one, answering 1–6 with file-level references the way this section does, then one task per phase of the scope order. Not a single task: the first phase changes the schema and the platform command path, and reviewing that inside a feature that also ships a control loop is how both get less attention than they need.
