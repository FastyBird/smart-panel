...
5. **What serialises, and around what.** Per *device*, not per loop — a heater and a cooler evaluating concurrently each see the other inactive and each energise. `DeviceStructureLockService` is the existing shape to borrow from, not necessarily the instance. Note this only serialises what goes through it, which is what makes 4 a prerequisite rather than a detail.
    ...
    So the boundary is confirmation, not dispatch, and there are two ways to hold it: keep the serialisation through the round trip, which needs a policy for an actuator that never confirms or the device stops evaluating entirely; or represent outstanding commands as interlock state, so a pending `heater=on` counts as active for the interlock while `status` still waits for the actuator. The second keeps 2 intact and is the smaller change to reporting; it costs the design an explicit answer for how long an unconfirmed command stays outstanding.
