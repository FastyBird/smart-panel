...
**Design requirement — read the classification table below before designing any control feedback.** Only the **“Shared state machine”** level gets the full treatment (locked/pending appearance *and* a defined rollback) — and even there, climate omits the rollback (see below). Today that is expressed mostly by disabled styling and by the value simply showing the desired number; a redesign is free to make it richer but must not remove it. Every level below it supports strictly less, and giving any of them more shows feedback the implementation cannot honour.

⚠️ **The six levels — do not flatten them.** This table is the **authoritative classification**; the settling table above supplies *timings only*. Adding a row there does not classify it — add it here too.

*Cross-references elsewhere name a level rather than numbering it, so inserting a row cannot silently redirect them.*

| Control | What it actually does | What to design |
|---|---|---|
| **1 — Shared state machine.** Device-detail controls that call `setPending`/`setSettling`; the lights / shading / climate **domain** controls; and the **hero display** when a role toggle drives it (i.e. the toggled role is the selected one — the *tile* is still the **“Landscape role tiles”** level) | Locks the control and tracks the desired value. On convergence → IDLE; **on timeout → MIXED, which stays locked** (see above); on failure → rollback — ⚠️ **except Climate, see below** | Full **locked/pending appearance + rollback**, **plus a distinct MIXED / "never confirmed" state**. |
| **2 — Landscape role tiles** (lights **and** shading) | The icon tap is **never lock-gated** — the tile stays tappable throughout. It writes a **local optimistic value** and starts a timer; when the timer expires the local value is dropped and the real value shows again.<br>· **Lights** — on/off flag, **3000 ms**<br>· **Shading** — target position (≥ 50 → close to 0, else open to 100), **2000 ms**<br><br>Whether the state machine is involved depends on *which* role was tapped:<br>· **Selected role** → also drives the hero, so the **hero display** follows the **“Shared state machine”** level<br>· **Non-selected role** → skips the hero entirely. For lights that means `setPending` *and* the failure-path `setIdle` are both skipped (both sit inside `if (updateHero)`), so there is no rollback anywhere.<br><br>On failure: an error toast, while the tile **keeps showing the optimistic value** until its own timer expires | **Immediate value change on the tile, always tappable, no lock, no rollback** — for both domains and for both selected and non-selected roles. If the tapped role is also the selected one, the *hero* additionally gets the **“Shared state machine”** treatment; the tile itself never does. |
...
| **4 — Media domain transport** (play / pause / stop) | The displayed playback state flips **immediately** and socket truth is suppressed for 3 s, after which the device is re-read. Fire-and-forget: not awaited, **no failure branch, no rollback**, buttons stay **tappable** | An **immediate active-state change** — nothing more. No lock, no spinner, no disabled state, no rollback animation. |
| **5 — Room-overview quick actions** (lights / climate / shading) | `_setOptimistic` swaps the **displayed card value** for up to 5 s. Buttons stay tappable, nothing locks, a failure just drops the override | An **updated value** on the card. No lock, no disabled state, no rollback animation. |
| **6 — Room-overview card media buttons** | Direct property write. No flip, no suppression, no lock — nothing changes until real state arrives (§8.1) | **No pending affordance at all.** |
...
5. **Six states per screen** (§17), plus control feedback matched to the **six-level classification table** in §17 — which is authoritative, while the settling table supplies timings only. Only the **“Shared state machine”** level gets locked/pending + rollback — **and it also needs a distinct MIXED / "never confirmed" presentation**, which today is indistinguishable from settling. Every level below it supports strictly less, down to no affordance at all. Never infer a level from the settling table.
