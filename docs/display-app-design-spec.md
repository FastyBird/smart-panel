...
5. **Six states per screen** (§17), plus control feedback matched to the **six-level classification table** in §17 — which is authoritative, while the settling table supplies timings only. Only level 1 gets locked/pending + rollback — **and it also needs a distinct MIXED / "never confirmed" presentation**, which today is indistinguishable from settling. Levels 2–6 each support strictly less, down to no affordance at all. Never infer a level from the settling table.
