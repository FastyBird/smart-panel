...
|---|---|---|
| **1 — Shared state machine.** Device-detail controls that call `setPending`/`setSettling`; the lights / shading / climate **domain** controls; and role toggles **only when they drive the hero** (i.e. the toggled role is the selected one) | Locks the control and tracks the desired value. On convergence → IDLE; **on timeout → MIXED, which stays locked** (see above); on failure → rollback — ⚠️ **except Climate, see below** | Full **locked/pending appearance + rollback**, **plus a distinct MIXED / "never confirmed" state**. |
| **2 — Landscape role tile, non-selected role.** Tapping the icon on a role tile that is *not* the currently selected role | Sets a **local flag** and starts a 3 s timer, then calls `_toggleRoleLights(updateHero: false)`. Because `updateHero` is false, **both** the `setPending` and the failure-path `setIdle` are skipped — the tile never enters the state machine and never rolls back. It is never locked, so it stays tappable; on failure it shows an error toast while the optimistic icon **persists for the full 3 s** | **Immediate icon flip, stays tappable, no lock.** On failure: an error toast *and the icon stays wrong* until the timer expires. Do not design a rollback here. |
