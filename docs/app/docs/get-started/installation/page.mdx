...
<Steps>
	...
	<Callout type="info">
		If the archive is valid, the output will show:
		<code>smart-panel.tar.gz: OK</code>
	</Callout>
	...
	<Callout type="default">
		Want to rotate the display? Add the `-r` (physical rotation) flag to the `ExecStart` line. For example:
